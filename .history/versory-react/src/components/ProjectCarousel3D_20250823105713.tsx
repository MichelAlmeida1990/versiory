      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-versiory-pink/20 to-versiory-green/20 backdrop-blur-sm border border-versiory-pink/30 rounded-full px-6 py-3 mb-6">
            ...
            <span className="text-versiory-pink font-semibold">Carrossel 3D</span>
          </div>
          ...
          <p className={`text-xl max-w-3xl mx-auto ${theme === 'dark' ? 'text-white/80' : 'text-gray-700'}`}>
            Carrossel de cartas 3D com projetos reais desenvolvidos com tecnologias modernas.
          </p>
        </motion.div>
            ...
            {categories.map((category, index) => (
              ...
              >
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={() => setFilter(category.key as 'web' | 'mobile' | 'system' | 'ecommerce' | 'professional')}
        ...
        >
                ...
                >
                  ...
                  >
                    ...
                    <div className="relative h-48 overflow-hidden">
                      ...
                      {/* Featured Badge */}
                      {project.featured && (
                        <div className="absolute top-3 left-3 bg-gradient-to-r from-versiory-green to-versiory-azure text-black px-2 py-1 rounded-full text-xs font-bold flex items-center space-x-1">
                          <Star size={10} />
                          <span>Destaque</span>
                        </div>
                      )}
                      ...
                      {/* Category Badge */}
                      <div className="absolute top-3 right-3 bg-versiory-black/80 backdrop-blur-sm rounded-lg p-2 flex items-center space-x-1">
                        {project.icon}
                        <span className="text-white/90 text-xs font-medium">
                          {categories.find(cat => cat.key === project.category)?.label.split(' ')[0]}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-6 space-y-4">
                      <div>
                        ...
                        <motion.a
                          ...
                          className={`text-sm leading-relaxed font-bold cursor-pointer transition-all duration-300 hover:text-versiory-green ${
                            theme === 'dark' ? 'text-white/70 hover:text-versiory-green' : 'text-gray-600 hover:text-versiory-green'
                          }`}
                        >
                      ...
                      {isActive && (
                        ...
                        >
                          <motion.a
                            ...
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            className="flex-1 bg-versiory-green text-black px-3 py-2 rounded-lg font-semibold text-sm flex items-center justify-center space-x-1"
                          >
                          ...
                          <motion.a
                            ...
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            className="flex-1 border border-versiory-azure text-versiory-azure px-3 py-2 rounded-lg font-semibold text-sm flex items-center justify-center space-x-1 hover:bg-versiory-azure hover:text-black transition-colors"
                          >
                            <Github size={14} />
                            <span>Code</span>
                          </motion.a>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 border border-versiory-azure/60 rounded-full flex items-center justify-center text-versiory-azure hover:border-versiory-green/70 transition-all duration-300 bg-versiory-black/30 backdrop-blur-sm"
          >
            <ChevronLeft size={24} />
          </motion.button>
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 border border-versiory-azure/60 rounded-full flex items-center justify-center text-versiory-azure hover:border-versiory-green/70 transition-all duration-300 bg-versiory-black/30 backdrop-blur-sm"
          >

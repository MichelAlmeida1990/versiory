...
const About = () => {
  ...
  const valuesStagger = useStaggerAnimation(values, 0.15);
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
          ...
          <div ref={visionMissionAnimation.ref} style={visionMissionAnimation.style} className="space-y-8">
            <motion.div
              ...
              className={`backdrop-blur-sm border border-versory-azure/60 rounded-2xl p-8 ${theme === 'dark' ? 'bg-gradient-to-br from-versory-blue/10 to-versory-azure/10' : 'bg-blue-light/70'}`}
            >
              <h3 className="text-2xl font-bold bg-gradient-to-r from-versory-green to-versory-azure bg-clip-text text-transparent mb-4">Nossa Visão</h3>
            ...
            <motion.div
              whileHover={{ scale: 1.02 }}
              className={`backdrop-blur-sm border border-versory-pink/60 rounded-2xl p-8 ${theme === 'dark' ? 'bg-gradient-to-br from-versory-pink/10 to-versory-green/10' : 'bg-blue-light/70'}`}
            >
              <h3 className="text-2xl font-bold bg-gradient-to-r from-versory-green to-versory-azure bg-clip-text text-transparent mb-4">Nossa Missão</h3>
              <p className={`leading-relaxed ${theme === 'dark' ? 'text-white/90' : 'text-gray-800'}`}>
          ...
          <div ref={ctaAnimation.ref} style={ctaAnimation.style} className="text-center">
            <motion.div
              ...
              className={`backdrop-blur-sm border border-versory-green/60 rounded-2xl p-8 ${theme === 'dark' ? 'bg-gradient-to-br from-versory-green/10 to-versory-azure/10' : 'bg-blue-light/70'}`}
            >
        ...
        <div ref={valuesAnimation.ref} style={valuesAnimation.style}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((value, index) => (
              <div key={value.letter} ref={valuesStagger.refs[index]} style={valuesStagger.getStaggerStyle(index)}>
                <motion.div
                  whileHover={{ scale: 1.05, y: -5 }}
                  className={`backdrop-blur-sm border border-versory-azure/60 rounded-2xl p-6 h-full ${theme === 'dark' ? 'bg-gradient-to-br from-versory-blue/5 to-versory-azure/5' : 'bg-blue-light/70'}`}
                >
                  ...
                  <h4 className={`text-xl font-bold mb-2 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                    <span className="bg-gradient-to-r from-versory-green to-versory-azure bg-clip-text text-transparent">{value.letter}</span>{value.title}
                  </h4>

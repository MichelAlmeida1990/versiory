...
const Services = () => {
  ...
  return (
    <section id="servicos" className={`py-20 ${theme === 'dark' ? 'bg-gradient-to-b from-versory-black to-versory-blue/5' : 'bg-gradient-to-b from-gray-50 to-blue-50/20'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal direction="up" className="text-center mb-16">
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            className="inline-flex items-center space-x-2 bg-gradient-to-r from-versory-blue/20 to-versory-azure/20 backdrop-blur-sm border border-versory-azure/30 rounded-full px-6 py-3 mb-6"
          >
        ...
        <ScrollReveal direction="up" delay={0.3}>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              ...
              >
                {/* Card Principal */}
                <div className={`relative overflow-hidden rounded-2xl p-8 h-full transition-all duration-500 backdrop-blur-sm border group-hover:border-white/30 ${theme === 'dark' ? `${service.gradient} bg-opacity-20 border-white/10` : 'bg-white/10 border-versory-azure/20'}`}>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-2xl font-bold text-white group-hover:text-versory-green transition-colors">
                      {service.title}
                    </h3>

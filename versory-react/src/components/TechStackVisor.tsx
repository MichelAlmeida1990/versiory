...
const TechStackVisor = () => {
  ...
  const techStack: TechIcon[] = useMemo(
    () => [
      {
        name: 'React',
        icon: <Atom size={32} />,
        ...
        {isOpen && (
          ...
          >
            ...
            >
              ...
              {/* Visor Container */}
              <div
                className={`relative overflow-hidden border border-versiory-azure/60 rounded-xl p-6 ${theme === 'dark' ? 'bg-versiory-black/30' : 'bg-blue-light/70'}`}
              >
                {/* Marquee Container */}
                <div className='flex space-x-8 animate-marquee'>
                  ...
                  {techStack.map((tech, index) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-xs font-medium text-center max-w-16 ${theme === 'dark' ? 'text-white/80' : 'text-gray-700'}`}
                      >
                        {tech.name}
                      </span>
                    </motion.div>
                  ...
                  {techStack
                    ...
                    .map((tech, index) => (
                      ...
                      >
                        ...
                        <span
                          className={`text-xs font-medium text-center max-w-12 ${theme === 'dark' ? 'text-white/60' : 'text-gray-600'}`}
                        >
                          {tech.name}
                        </span>
                      </motion.div>
              ...
              <div className='mt-8 text-center'>
                <p className='text-versiory-azure/80 text-sm'>
                  Passe o mouse sobre os ícones para ver as animações
                  interativas
                </p>
                <div
                  className={`flex justify-center space-x-4 mt-4 text-xs ${theme === 'dark' ? 'text-white/60' : 'text-gray-600'}`}
                >
                  <span>🔄 Rotação</span>
                  <span>💓 Pulso</span>
                  <span>🌊 Flutuação</span>
